...
The MAC is read from the agent's `/health` (straight from `/sys/class/net/<iface>/address`), once during setup and again on every reconnect — not typed in, and not something a motherboard or NIC swap can leave stale, since the next successful connect just picks up the new one. An agent too old to report it (or a NIC ethtool can't read) leaves wake capability off, same as an unconfigured MAC always has:
