...
- 12 games were installed on this box at research time; non-game infrastructure appids (Proton builds, Steam Linux Runtime, Steamworks Common Redistributables) naturally sort to the bottom/get filtered by the same "never really played" `LastPlayed` check — no special-case exclusion needed.
